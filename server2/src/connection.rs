...
use signaler_protocol::{ConnectionCommand, Credentials, SessionMessage, SessionDescription};
...
impl Connection {
    ...
    fn associate_session(&self, credentials: Credentials, ctx: &mut Context<Self>) {}
}
...
impl Actor for Connection {
    async fn started(&mut self, ctx: &mut xactor::Context<Self>) -> xactor::Result<()> {
        ...
        if let Some(ws_receiver) = self.ws_receiver.take() {
            ...
            self.send(SessionMessage::Welcome {
